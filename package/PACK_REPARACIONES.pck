...
PROCEDURE insertar_usuario(
    ...
    v_count NUMBER;
  BEGIN
    SELECT COUNT(1) INTO v_count FROM generos WHERE id = p_genero_id;
    IF v_count = 0 THEN
      p_mensaje := 'Error: Género no válido';
      RETURN;
    END IF;

    SELECT COUNT(1) INTO v_count FROM roles WHERE id = p_rol_id;
    IF v_count = 0 THEN
  ...
  PROCEDURE insertar_equipo(
    ...
    v_count NUMBER;
  BEGIN
    SELECT COUNT(1) INTO v_count FROM estados WHERE id = p_estado_id;
    IF v_count = 0 THEN
